...
public class NewsServiceImpl implements NewsService {
	...
	public boolean addNews(News news) {
		Optional<UserNews> optionalUserNewsobj = newsRepo.findById(news.getAuthor());		
		UserNews newUserNews = null;
		if(optionalUserNewsobj.isPresent()) {
			...
			if(newUserNews!=null) {
				...
				newUserNews = new UserNews();
				newUserNews.setNewslist(new ArrayList<News>());
	...
	public boolean deleteNews(String userId, int newsId) {
		...
		if(userNewsObj.isPresent()) {
			...
			if(newsObj.isPresent()) {
				userNewsObj.get().getNewslist().remove(newsObj.get());
				newsRepo.save(userNewsObj.get());
